...
<style>
  ...
  * { margin: 0; padding: 0; box-sizing: border-box; }
  ...
  /* ── WATCH & NOTES ── */
  #page-watch { max-width: 100% !important; padding: 1rem 1.5rem !important; }
  .watch-header { display: flex; align-items: center; gap: 1rem; margin-bottom: 1rem; flex-wrap: wrap; }
  .watch-header .section-title { margin-bottom: 0; }
  .watch-url-row { display: flex; gap: 0.75rem; flex: 1; min-width: 260px; }
  .watch-url-input { flex: 1; background: var(--card); border: 1px solid var(--border); border-radius: 12px; padding: 0.75rem 1.1rem; color: var(--text); font-family: 'DM Sans', sans-serif; font-size: 0.9rem; outline: none; transition: border 0.2s; }
  .watch-url-input:focus { border-color: var(--accent); }
  .watch-url-input::placeholder { color: var(--muted); }
  .watch-split { display: grid; grid-template-columns: 1fr 380px; gap: 1.2rem; height: calc(100vh - 160px); min-height: 480px; }
  @media (max-width: 860px) { .watch-split { grid-template-columns: 1fr; height: auto; } }
  .watch-player { background: #000; border-radius: 16px; overflow: hidden; position: relative; display: flex; align-items: center; justify-content: center; }
  .watch-player iframe { width: 100%; height: 100%; border: none; }
  .watch-placeholder { text-align: center; color: var(--muted); }
  .watch-placeholder .icon { font-size: 3.5rem; margin-bottom: 1rem; }
  .watch-placeholder p { font-size: 0.9rem; }
  .watch-notes-panel { background: var(--card); border: 1px solid var(--border); border-radius: 16px; display: flex; flex-direction: column; overflow: hidden; }
  .wnp-header { padding: 1rem 1.2rem; border-bottom: 1px solid var(--border); display: flex; align-items: center; justify-content: space-between; }
  .wnp-header h3 { font-family: 'Syne', sans-serif; font-size: 1rem; font-weight: 700; }
  .wnp-count { color: var(--muted); font-size: 0.8rem; }
  .wnp-input-area { padding: 1rem 1.2rem; border-bottom: 1px solid var(--border); }
  .wnp-title-input { width: 100%; background: var(--surface); border: 1px solid var(--border); border-radius: 8px; padding: 0.6rem 0.9rem; color: var(--text); font-family: 'DM Sans', sans-serif; font-size: 0.85rem; outline: none; margin-bottom: 0.6rem; transition: border 0.2s; }
  .wnp-title-input:focus { border-color: var(--accent); }
  .wnp-title-input::placeholder { color: var(--muted); }
  .wnp-textarea { width: 100%; background: var(--surface); border: 1px solid var(--border); border-radius: 8px; padding: 0.6rem 0.9rem; color: var(--text); font-family: 'DM Sans', sans-serif; font-size: 0.85rem; outline: none; resize: none; height: 90px; transition: border 0.2s; margin-bottom: 0.6rem; }
  .wnp-textarea:focus { border-color: var(--accent); }
  .wnp-textarea::placeholder { color: var(--muted); }
  .wnp-actions { display: flex; gap: 0.5rem; align-items: center; }
  .wnp-ts { color: var(--accent2); font-size: 0.75rem; font-family: 'Syne', sans-serif; cursor: pointer; background: rgba(247,194,106,0.1); border: 1px solid rgba(247,194,106,0.3); border-radius: 6px; padding: 3px 8px; transition: all 0.2s; }
  .wnp-ts:hover { background: rgba(247,194,106,0.25); }
  .wnp-list { flex: 1; overflow-y: auto; padding: 0.8rem 1.2rem; display: flex; flex-direction: column; gap: 0.7rem; }
  .wnp-note { background: var(--surface); border: 1px solid var(--border); border-radius: 10px; padding: 0.85rem 1rem; position: relative; }
  .wnp-note-top { display: flex; justify-content: space-between; align-items: center; margin-bottom: 0.3rem; }
  .wnp-note-title { font-family: 'Syne', sans-serif; font-size: 0.85rem; font-weight: 600; }
  .wnp-note-stamp { font-size: 0.72rem; color: var(--accent2); font-family: 'Syne', sans-serif; cursor: pointer; }
  .wnp-note-stamp:hover { text-decoration: underline; }
  .wnp-note-body { font-size: 0.82rem; color: var(--muted); line-height: 1.5; white-space: pre-wrap; }
  .wnp-note-del { position: absolute; top: 0.6rem; right: 0.6rem; background: none; border: none; color: var(--border); cursor: pointer; font-size: 0.9rem; transition: color 0.2s; }
  .wnp-note-del:hover { color: var(--red); }
  .wnp-empty { text-align: center; color: var(--muted); font-size: 0.85rem; padding: 2rem 0; }
...
<nav>
  ...
  <button class="nav-btn" onclick="showPage('watch', this)">🎬 Watch & Notes</button>
...
<div id="page-home" class="page active">
  <div class="hero">
    ...
    <div class="hero-cards">
      ...
      <div class="hero-card" onclick="nav('watch')"><div class="icon">🎬</div><h3>Watch & Notes</h3></div>
...
<div id="page-watch" class="page">
  <div class="watch-header">
    <div class="section-title">🎬 Watch &amp; <span>Notes</span></div>
    <div class="watch-url-row">
      <input class="watch-url-input" id="watchUrlInput" placeholder="Paste YouTube link or video ID…" onkeydown="if(event.key==='Enter') loadWatchVideo()">
      <button class="btn" onclick="loadWatchVideo()">Load</button>
    </div>
  </div>
  <div class="watch-split">
    <div class="watch-player" id="watchPlayer">
      <div class="watch-placeholder">
        ...
        <p>Paste a YouTube link above<br>to start watching</p>
      </div>
    </div>
    <div class="watch-notes-panel">
      <div class="wnp-header">
        <h3>📝 Video Notes</h3>
        <span class="wnp-count" id="wnpCount">0 notes</span>
      </div>
      <div class="wnp-input-area">
        <input class="wnp-title-input" id="wnpTitle" placeholder="Note title (optional)…">
        <textarea class="wnp-textarea" id="wnpText" placeholder="Type your note… press Ctrl+Enter to save"></textarea>
        <div class="wnp-actions">
          <button class="btn btn-sm" onclick="saveWatchNote()">+ Save Note</button>
          <span class="wnp-ts" id="wnpTs" title="Click to insert current timestamp" onclick="insertTimestamp()">⏱ Timestamp</span>
          <button class="btn btn-sm btn-outline" onclick="exportWatchNotes()">⬇ Export</button>
        </div>
      </div>
      <div class="wnp-list" id="wnpList">
        <div class="wnp-empty">No notes yet.<br>Start watching and jot down ideas!</div>
      </div>
    </div>
  </div>
</div>
...
function loadWatchVideo() {
  const raw = document.getElementById('watchUrlInput').value;
  const vid = extractVideoId(raw);
  if (!vid) { alert('Please enter a valid YouTube URL or video ID.'); return; }
  watchVideoId = vid;
  const container = document.getElementById('watchPlayer');
  container.innerHTML = '<div id="ytPlayerEl"></div>';

  if (typeof YT !== 'undefined' && YT.Player) {
    createYTPlayer(vid);
  } else {
    // Fallback: embed iframe directly
    container.innerHTML = `<iframe src="https://www.youtube.com/embed/${vid}?enablejsapi=1" allow="autoplay; encrypted-media" allowfullscreen style="width:100%;height:100%;border:none;"></iframe>`;
    watchPlayer = null;
  }
}

function createYTPlayer(vid) {
  watchPlayer = new YT.Player('ytPlayerEl', {
    videoId: vid,
...
function getCurrentTime() {
  if (watchPlayer && typeof watchPlayer.getCurrentTime === 'function') {
    const secs = Math.floor(watchPlayer.getCurrentTime());
...
function insertTimestamp() {
  ...
  const ta = document.getElementById('wnpText');
  if (ts) {
    const cur = ta.value;
    const insert = `[${ts}] `;
    ta.value = cur + insert;
  } else {
    alert('Load a video first to insert a timestamp.');
  }
  ta.focus();
}

function saveWatchNote() {
  const title = document.getElementById('wnpTitle').value.trim();
  const text = document.getElementById('wnpText').value.trim();
  if (!text) { alert('Write something before saving!'); return; }
  const ts = getCurrentTime();
  watchNotes.unshift({ id: watchNoteIdCounter++, title: title || 'Note', text, ts, videoId: watchVideoId });
  document.getElementById('wnpTitle').value = '';
  document.getElementById('wnpText').value = '';
  renderWatchNotes();
  // Also sync to the main notes API so it appears in the Notes tab
  fetch('/api/notes', {
    method: 'POST', headers: {'Content-Type': 'application/json'},
    body: JSON.stringify({ title: (title || 'Note') + (ts ? ` [${ts}]` : ''), content: text })
  });
}

function deleteWatchNote(id) {
  watchNotes = watchNotes.filter(n => n.id !== id);
  renderWatchNotes();
}

function seekTo(ts) {
  if (!ts || !watchPlayer || typeof watchPlayer.seekTo !== 'function') return;
  const [m, s] = ts.split(':').map(Number);
  watchPlayer.seekTo(m * 60 + s, true);
}

function renderWatchNotes() {
  const list = document.getElementById('wnpList');
  const count = document.getElementById('wnpCount');
  count.textContent = watchNotes.length + ' note' + (watchNotes.length !== 1 ? 's' : '');
  if (!watchNotes.length) {
    list.innerHTML = '<div class="wnp-empty">No notes yet.<br>Start watching and jot down ideas!</div>';
    return;
  }
  list.innerHTML = watchNotes.map(n => `
    <div class="wnp-note" id="wn-${n.id}">
      <div class="wnp-note-top">
        <span class="wnp-note-title">${n.title}</span>
        ${n.ts ? `<span class="wnp-note-stamp" onclick="seekTo('${n.ts}')" title="Jump to this moment">⏱ ${n.ts}</span>` : ''}
      </div>
      <div class="wnp-note-body">${n.text}</div>
      <button class="wnp-note-del" onclick="deleteWatchNote(${n.id})">🗑</button>
    </div>`).join('');
}

function exportWatchNotes() {
  if (!watchNotes.length) { alert('No notes to export!'); return; }
  const vidLine = watchVideoId ? `Video: https://www.youtube.com/watch?v=${watchVideoId}\n\n` : '';
  const text = vidLine + watchNotes.map(n =>
    `${n.ts ? '[' + n.ts + '] ' : ''}${n.title}\n${n.text}`
  ).join('\n\n---\n\n');
  const blob = new Blob([text], {type: 'text/plain'});
  const a = document.createElement('a');
  a.href = URL.createObjectURL(blob);
  a.download = 'watch-notes.txt';
  a.click();
}

// Ctrl+Enter to save note
document.addEventListener('keydown', function(e) {
  if (e.ctrlKey && e.key === 'Enter' && document.activeElement.id === 'wnpText') {
    saveWatchNote();
  }
});
